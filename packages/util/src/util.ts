 ...
 */
export function inRandomRate(value: number): boolean {
  value = +value;
...
export function randomId(length: number, { uniqueId = true, prefix = '' } = {}): string {
 ...
 */
export function delayTask(task: () => void, delay = 600): () => boolean {
  let running = false;
 ...
 * }]
 */
export function toTree<T extends Dict>(data: Array<T>, {
  id = 'id',
  parentId = 'parentId',
  rootParentId = 0,
  children = 'children',
...
/**
 * 在给定时间内将 start 值变化为 end，类似于一个动画过程，返回 stop 停止函数
 */
 ...
 */
export class LazyTasks<T extends Task> {
  tasks: Map<string, T>;

  finished: { [K: string]: boolean };
  ...
  // options: { cb(id) }
  start(id: string, options: Dict = {}): void {
    const task = this.tasks.get(id);
  ...
  stop(id: string, options: Dict = {}): void {
    const task = this.tasks.get(id);
